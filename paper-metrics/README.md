...
# reproduce everything (≈10 min, RF in t6 dominates)
./reproduce_all.sh
...
`advantages.parquet` for each env was generated by
`Analysis/RAW-ADV/generate_advantages.py` from the checkpoints inside the
matching zip. The eval-stats CSVs in `data/eval_stats/` are the per-step
