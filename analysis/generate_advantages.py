...
def generate_advantages(catalog_df: pd.DataFrame) -> pd.DataFrame:
...
def main() -> None:
    ...
    if "config_path" not in catalog_df.columns:
        raise ValueError("catalog CSV missing 'config_path' column — re-run catalog_checkpoints.py")

    result = generate_advantages(catalog_df)
    if result.empty:
        print("No advantages generated.")
    else:
        print(f"Generated {len(result):,} rows ({result['checkpoint_path'].nunique()} checkpoints)")
