...
if ADV_COLS:
    ...
    for (agent, actor), grp in adv_all_df.groupby(["hp.agent_name", "actor"]):
        print(f"\n=== {agent} | {actor} ===")
        temp_df = grp.groupby(["dataset", "config_index", "seed", "eval_step"])[
            "advantage"
        ].mean()
...
if ADV_COLS:
    q25 = adv_all_df.groupby(
        ["hp.agent_name", "dataset", "config_index", "seed", "actor", "eval_step"]
    )["advantage"].transform("quantile", 0.05)
    q75 = adv_all_df.groupby(
        ["hp.agent_name", "dataset", "config_index", "seed", "actor", "eval_step"]
    )["advantage"].transform("quantile", 0.95)

    adv_trimmed_df = adv_all_df[
        (adv_all_df["advantage"] >= q25) & (adv_all_df["advantage"] <= q75)
    ].copy()
    print(
        f"Trimmed dataframe: {len(adv_trimmed_df)} rows ({len(adv_all_df) - len(adv_trimmed_df)} outliers removed, {100 * (1 - len(adv_trimmed_df) / len(adv_all_df)):.1f}%)"
    )
